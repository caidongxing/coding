...
.spa {
  ...
  <form style="width:900px;margin:0 auto;margin-top: 28px;" action="${ctx}/StaffController/insertStaff.do" method="post">
    <table class="table table-bordered table-hover m10">
    
			<tr>
				<td class="tableleft">用户名:</td>
				<td><input id="username" name="username" type="text"><span class="spa spa1"></span></td>
				...
				<c:forEach items="${quartersTypes}" var="item">
					<option  value="${item.quarterstypeid}">${item.quarterstype}</option>						
				</c:forEach>
				</select>
				</td>
			</tr>
			<tr>
				...
				<td><input id="identitycard" name="identitycard" type="text"><span class="spa spa2"></span></td>
					<td class="tableleft">电话号码:</td>
				<td><input id="phone" name="phone" type="text"><span class="spa spa3"></span></td>
			</tr>									
			<tr>
			<td class="tableleft">性别:</td>
				...
				<td><input id="Password" name="Password" type="text"><span class="spa spa4"></span></td>
									
			</tr>
			<tr>
				<td class="tableleft">真实姓名:</td>
				<td><input id="realname" name="realname" type="text"><span class="spa spa5"></span></td>
			    <td class="tableleft">确认密码:</td>
				<td><input id="password" name="password" type="text"><span class="spa spa6"></span></td>
			</tr>				
		</table>
		...
		/********************** 聚焦提示 ************************/	
			$("input").focus(function(){
				...
				if($(this).is("#Password")){
					$(".spa4").text("请你输入登陆密码").css("color","#aaa")
					$(this).css("border","1px solid #aaa")
				}
				if($(this).is("#password")){
					$(".spa6").text("请你再次输入密码").css("color","#aaa")
					$(this).css("border","1px solid #aaa")
				}
			})
		  /*********************** 提交验证 ***************************/
			$("#subm").click(function(){
				...
				var sjh= /^((13[0-9])|(15[^4,\D])|(18[0-9]))\d{8}$/;		
				var mm=	/^\d{6}$/;
				if(sfz.test($("#identitycard").val()) && sjh.test($("#phone").val()) && $("#username").val()!="" && $("#realname").val()!="" && $("#Password").val()!="" && $("#password").val()!=""
				    && mm.test($("#Password").val()) && $("#Password").val()==$("#password").val()){
			     return true;					        				
			    	}else{
				if($("#username").val()==""){
						$(".spa1").text('请你输入用户名')
